...
describe('Database Insertion', function() {
    ...
    it('should insert data correctly into the all_data table', function() {
        // Prepare the SQL statement and insert the test data
        const insertStmt = db.prepare('INSERT INTO all_data (id, date, file, joules) VALUES (?, ?, ?, ?)');
        insertStmt.run(testID, testDate, testFilePath, testJoules);

        // Query the data to verify the insertion
        const selectStmt = db.prepare('SELECT * FROM all_data WHERE id = ?');
